...
function buildDiagnostics({ controller, app = {}, cli = {}, logLines = [] }) {
  ...
  const lines = [
    ...
    `Default access: ${s.access} · Jev key: ${yesNo(controller.smartRouter?.jev?.configured)}${controller.smartRouter?.jev?.health?.since ? ` (unavailable since ${new Date(controller.smartRouter.jev.health.since).toISOString()}: ${controller.smartRouter.jev.health.error})` : ''} · execution block: ${yesNo(controller.data.executionBlock)} · unconfirmed worker cleanups: ${controller.data.workerCleanup?.length || 0}`,
