...
3. Keep only comparable, independently verified records. Retain older verified rows if no newer comparable evidence exists. Remove superseded rows for the same source/model/effort/harness and explain removals. Do not combine versions. Do not double-count composites and their components. AA Coding Agent Index contains DeepSWE, Terminal-Bench and SWE-Atlas-QnA; AA Intelligence also overlaps some component evaluations.
...
5. API dollar costs do not measure ChatGPT/Claude/Cursor subscription quotas. Separate cost per task from price per token and preserve the tested API/harness. For ARC use costPerTaskUsd for v1/v2 and totalCostUsd for v3; never store the two in the same ambiguous field. Preserve standard vs provider-adapter harnesses. A leaderboard's total response time is not our app's latency. Never compare costs across different benchmark workloads as though they were identical. AA-AnalystAgent uses pass^5, not pass@1.
...
```json
{"schemaVersion":1,"updatedAt":"YYYY-MM-DD","records":[{"source":"deepswe","model":"exact-provider-model-id","effort":"xhigh","version":"1.1","harness":"mini-swe-agent","observedAt":"YYYY-MM-DD","evaluatedAt":null,"origin":"independent","url":"https://deepswe.datacurve.ai/","metrics":{"passPercent":74,"marginPercent":3,"costUsd":4.43}}]}
```
